-- Supabase Database Schema for Blu Blog Gen
...
CREATE TABLE IF NOT EXISTS projects (
    ...
    num_images_per_blog INTEGER DEFAULT 1 CHECK (num_images_per_blog >= 1 AND num_images_per_blog <= 4),
    serp_api_on BOOLEAN DEFAULT FALSE,
    enhanced_research BOOLEAN DEFAULT FALSE,
    serp_api_contents TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS api_keys (
    ...
    user_id UUID NOT NULL,
    name VARCHAR(255) NOT NULL,
    service VARCHAR(50) NOT NULL CHECK (service IN ('openai', 'gemini', 'serp', 'fal', 'other')),
...
GROUP BY p.id, p.name, p.total_blogs;
